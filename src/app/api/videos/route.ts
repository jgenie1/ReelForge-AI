...
export async function GET(req: Request) {
  try {
    ...
    // En mode dev sans DB, retourner des mockups
    const mockVideos = [
      ...
      { id: 'vid-2', title: "L'Arche Perdue de Salomon", niche: 'Histoires Bibliques', style: 'Illustration', duration: 45, status: 'RENDERING', videoUrl: null, viralScore: 78, views: 0, likes: 0, createdAt: new Date(Date.now() - 5 * 60000) },
    ...
    // Tenter une vraie query Prisma, fallback sur mockups si DB non disponible
    try {
      const where: any = {};
      ...
      const [videos, total] = await Promise.all([
        prisma.video.findMany({ where, take: limit, skip, orderBy: { createdAt: 'desc' }, include: { jobs: { take: 1, orderBy: { updatedAt: 'desc' } } } }),
