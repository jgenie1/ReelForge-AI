...
export default function SignUpPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    // Simulate signup check, then redirect to dashboard
    setTimeout(() => {
      setLoading(false);
    ...
    <div className="min-h-screen bg-[#0D0D0F] text-gray-100 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="flex flex-col items-center gap-2 mb-8">
          ...
          <p className="text-gray-400 text-xs font-medium">14 jours d'essai gratuit inclus (50 crédits)</p>
        </div>

        {/* Card */}
        <div className="glass-panel border border-border bg-card/25 rounded-3xl p-8 shadow-2xl">
          <h2 className="text-xl font-bold text-center text-white mb-6">Créer un compte ReelForge AI</h2>
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">ou s'inscrire par email</span>
            <div className="h-[1px] bg-border/80 flex-grow"></div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading ? "Création..." : "S'inscrire"} <ArrowRight className="w-4 h-4" />
            </button>
          </form>

          {/* Guarantee badge */}
          <div className="flex items-center gap-2 mt-4 p-2.5 bg-black/30 border border-border rounded-xl text-[10px] text-gray-400 font-medium">
            <ShieldCheck className="w-4 h-4 text-emerald-400 flex-shrink-0" />
            <span>Aucune carte bancaire requise. Annulation en 1 clic.</span>
          </div>
